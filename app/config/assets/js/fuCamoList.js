...
var participants, masterFamList, date, bairro, tabz, zone, houseGroup, assistant;
function display() {
...
function getList() {
    ...
    var varNamesIncl = "I.BAIRRO, I.CAMO, I.HOUSEGRP, I.TABZ, ";
    ...
    var sql = "SELECT " + varNamesIncl + varNamesHH + varNamesFU + 
        ...
        " WHERE I.BAIRRO = " + bairro + " AND I.TABZ = " + tabz + " AND I.HOUSEGRP = '" + houseGroup + "' AND I.OBS_IDADE IS NULL AND (I.ACCEPT != 2 OR I.ACCEPT IS NULL) AND I.ESTADO IS NOT NULL" +
    ...
    var successFn = function( result ) {
        ...
        for (var row = 0; row < result.getCount(); row++) {
            ...
            var p = {type: 'participant', savepoint, FUDate, FUEnd, LastFU, BAIRRO, CAMO, HOUSEGRP, TABZ, DATEX, COVID, DATSEG, ESTADO, FU, LASTINTERVIEW, POSSIVEL, TESTRESUL};
            participants.push(p);
        }
...
function initButtons() {
    ...
    $.each(listFromMaster, function() {
        ...
        btn.on("click", function() {
            var queryParams = util.setQuerystringParams(date, that.bairro, that.tabz, that.zone, that.houseGroup, that.camo, null, null, null, assistant);
...
function getCount(camo) {
    ...
    var total = participants.filter(person => person.BAIRRO == bairro & person.TABZ == tabz & person.HOUSEGRP == houseGroup & person.CAMO == camo & (person.FUDate <= today & person.LastFU < person.FUEnd & ((person.ESTADO != "2" & person.ESTADO != "3") | person.POSSIVEL == "2" | person.TESTERESUL == "3") | person.DATSEG == todayAdate)).length;
